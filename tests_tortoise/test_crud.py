"""CRUD tests for Tortoise ORM bookstore."""
...
@pytest.mark.asyncio
async def test_author_not_found(client):
    resp = await client.get("/api/authors/99999")
    assert resp.status_code == 404
...
async def test_create_book(client, author_id):
    resp = await client.post("/api/books", json={
        "title": "New Book", "price": 15.99, "author_id": author_id
    })
    assert resp.status_code == 201
    data = resp.json()
...
async def test_update_book(client, book_id, author_id):
    resp = await client.put(f"/api/books/{book_id}", json={
        "title": "Updated Title", "price": 19.99, "author_id": author_id
    })
    assert resp.status_code == 200
...
async def test_create_review(client, book_id):
    resp = await client.post("/api/reviews", json={
        "book_id": book_id, "reviewer_name": "Bob", "rating": 5, "comment": "Great!"
    })
    assert resp.status_code == 201
...
async def test_review_validation(client, book_id):
    resp = await client.post("/api/reviews", json={
        "book_id": book_id, "reviewer_name": "Bob", "rating": 6
    })
